...
type AuthStorer interface {
	...
	GetAccountByUUID(uuid string) (*models.Account, error)
...
type Mailer interface {
	...
	UnatuhNorification(name, address string, c email.ContentUnauthNotif) error
}
...
func logReq(r *http.Request) logrus.FieldLogger {
...
func (rs *Resource) login(w http.ResponseWriter, r *http.Request) {
	...
	if err := render.Bind(r, body); err != nil {
		logReq(r).WithField("email", body.Email).Warn(err)
	...
	/*
		...
		log.Println("Possible pass", string(hash))
		log.Println("Hash pass", acc.Pass)
	*/
	pwd := []byte(body.Pass)
	err = bcrypt.CompareHashAndPassword([]byte(acc.Pass), pwd)
	...
	// Accces / Refresh token generation process
...
func (rs *Resource) token(w http.ResponseWriter, r *http.Request) {
	...
	acc, err := rs.Store.GetAccountByUUID(id)
	if err != nil {
...
func (rs *Resource) refresh(w http.ResponseWriter, r *http.Request) {
	...
	acc, err := rs.Store.GetAccountByUUID(token.AccountID)
